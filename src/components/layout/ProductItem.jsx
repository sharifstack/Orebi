...
const Li = ({ name, iconName }) => {
  return (
    <li className="flex justify-end md:gap-3 gap-0 items-center cursor-pointer font-DM transition-all duration-150 text-ptext text-base font-bold hover:text-Primary">
      {name} {iconName}
    </li>
  );
};
      ...
      <div className="image w-full h-[370px] relative group">
        ...
        <div className="overlay absolute left-0 bottom-0 bg-white/40 w-full px-7 py-6 opacity-0 invisible transition-all duration-300 group-hover:opacity-100 group-hover:visible ">
          <ul className="flex flex-col gap-3">
            ...
            <Li
              name="Compare"
              iconName={<FaArrowsRotate className="text-Primary" />}
            />
            <Li
              name="Add to Cart"
        ...
        <button
          className={`absolute top-5 left-5  w-[92px] bg-Primary text-white font-DM text-sm font-bold ${
            Offershow ? "py-2" : "py-0"
          }`}
        >
          {Offer}% off
